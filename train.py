...
experience_sample = namedtuple('experience_sample',
...
EPISODE = int(10000)  #number of game time the agent play
GAMMA = 0.9
...
for current_episode in range(EPISODE):
    ...
    while True:
        game_step += 1
        total_game_step += 1
        action = policy.act(state, total_game_step,
                            isTrain=True).to(DEVICE)  # sample an action
        ...
        if replay_buffer.lenth(
        ) > BATCH_SIZE:  # only optimize when replay buffer have sufficient number of data
            ...
            q_value_array = policy(
                state_batch
            )  # get 4 value of all actions [V(a0), V(a1), V(a2), V(a3)]
            ...
            terminal_mask = torch.tensor(tuple(
                map(lambda a: a is not None, samples.next_state)),
                                         device=DEVICE)
            next_state_batch = torch.cat([
                a for a in samples.next_state if a is not None
            ])  # select the non-teminal next_state
            # initialize Q(S(j+1)) as 0
            q_value_next = torch.zeros(BATCH_SIZE).to(DEVICE)
            q_value_next[terminal_mask] = (torch.max(target(next_state_batch),
                                                     1)[0]).detach()
